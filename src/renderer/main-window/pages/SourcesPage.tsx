...
export function SourcesPage(): JSX.Element {
  ...
  return <div className="sources-page">
    ...
    <p className="sources-formats muted">文件：TXT、Markdown、CSV、HTML、DOCX、XLSX、PPTX、PDF、EPUB、SRT/VTT 字幕。图片与扫描页暂不做文字识别；网页仅读取可获取内容。</p>
    ...
    <div className="sources-workspace">
      ...
      {source ? <div className="source-document">
        ...
        <div className="source-analysis"><label className="checkbox-label"><input type="checkbox" checked={useAi} disabled={sourceBusy} onChange={(event) => setUseAi(event.target.checked)} />使用 AI 补充专业术语</label><button className="button-secondary" disabled={busy || sourceBusy || status.paused} onClick={() => void run(() => api().sourceAnalyze({ id: source.id, useAi }))}>分词并识别全文</button>{source.kind !== 'window' && <button className="text-button" onClick={() => void run(() => api().sourceOpenLocation({ id: source.id }))}>{source.kind === 'file' ? '定位原文件' : '打开原网页'}</button>}</div>
